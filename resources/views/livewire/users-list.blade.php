...
<x-flashmessage />
<x-delete-still-modal internalmsg="Do you really want to delete User? This process cannot be undone"/>
<x-update-user-modal :accounts="$accounts"/>
<x-add-user-modal :accounts="$accounts"/>
...
<div class="flex w-full">
    <div class="mr-auto pt-2 relative mb-4 text-gray-600">
      ...
      <button 
      x-on:click="$dispatch('open-addmodal')" 
    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        @foreach($Users as $user)
            ...
            <tr wire:key="user-{{$user->id}}" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                ...
                <td class="px-6 py-4">
                {{$user->account->acountname}}
                </td>

               
              <td>
                <div class="w-full sm:w-1/8 text-right m-5 max-w-fit">
                <button type="button" wire:click="populatemodal('{{$user->id}}','{{$user->firstname}}',
                '{{$user->lastname}}','{{$user->email }}','{{$user->userType}}',' {{$user->account->id}}')"
                ...
                <div class="w-full sm:w-1/8 text-right m-5 max-w-fit">
                <button wire:key="user-{{$user->id}}" type="button" x-on:click="showdelmodal=!showdelmodal,showMessage=true,id={{$user->id}}" class="text-red-600 inline-flex items-center
                     hover:text-white 
